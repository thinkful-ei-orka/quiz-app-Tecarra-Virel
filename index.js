...
function generateQuizOptionsString(index) {
  const items = STORE.questions[index].choices.map((choice, idx) => "<button type='button' aria-pressed='false' class='form-option js-form-option' name='answer' value='" + STORE.questions[index].choices[idx] + "'> " + STORE.questions[index].choices[idx] + "</button>");
  return items.join("");
}
